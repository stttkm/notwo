...
/**
 * Tailwind v4 color palette with OKLCH values.
 * Source: tweakcn (accurate Tailwind CSS values)
 */
...
type ColorScale = Record<TailwindShade, string>;

// Tailwind colors are hand-tuned by designers and cannot be computed algorithmically
export const TAILWIND_PALETTE: Record<string, ColorScale> = {
  red: {
...
export function getColor(colorName: string, shade: TailwindShade): string {
  return TAILWIND_PALETTE[colorName]?.[shade] ?? "oklch(0.5 0 0)";
}

/** Get foreground color (white/black) based on shade. */
export function getForegroundForShade(shade: TailwindShade): string {
  return shade >= 500 ? "oklch(0.98 0 0)" : "oklch(0.15 0 0)";
}
